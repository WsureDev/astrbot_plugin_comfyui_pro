...
本插件的推荐生图入口是 LLM 工具调用，提示词由当前 workflow 模型资料和用户系统提示词共同决定。

## 介绍
一个功能强大的 AstrBot 插件，旨在将你本地的 **ComfyUI** 无缝集成到聊天机器人中。

它不仅支持基础的文生图，更创新的引入了 LLM（大语言模型）作为“画图提示词编写助手”，结合当前 workflow 的模型资料和用户系统提示词生成提示词。同时，它对 ComfyUI 用户极其友好，支持便捷地导入和切换你自己的工作流，并提供了完善的权限控制和敏感词过滤系统。
...
### 🤖 智能 LLM 绘图
*   **自然语言生图**: 用户只需说“帮我画一个...”，LLM 即根据当前 workflow 的模型资料和用户系统提示词编写提示词并触发绘图。
*   **指令生图**: 为高级用户保留了传统的 `/画图` 指令，可直接传入目标模型所需的提示词格式。
*   **高度可定制**: 你可以随时在后台修改 System Prompt，定制你的“AI 绘画助手”人设和回复风格。
*   **模型感知提示词**: LLM 可通过 `comfyui_workflows` 读取各 workflow 的静态模型声明，再结合用户 System Prompt 和实例补充配置决定提示词形式。
*   **工具调用**: 需要出图时使用 `comfyui_txt2img`，提示词格式由目标模型资料和用户系统提示词决定。
...
**Q: 为什么工具调用后没有出图？**
A: 请检查工具返回的错误信息、workflow 模型声明、输入节点和 ComfyUI 服务状态。
